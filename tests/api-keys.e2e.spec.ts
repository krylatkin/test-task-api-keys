...
test('disables and re-enables an API key', async ({ page }, testInfo) => {
  ...
  if (projectName === 'desktop') {
    await expect(getApiKeyItem(page, name, projectName)).toContainText(
      'Revoked',
    );
  }
...
test('selects a different card on mobile', async ({ page }, testInfo) => {
  test.skip(
    ...
    'Mobile-only card interaction.',
  );

  const initialCard = getApiKeyItem(
    page,
    'ai_inference_key',
    testInfo.project.name,
  );
  const targetCard = getApiKeyItem(
    page,
    'model_training_key',
    testInfo.project.name,
  );
