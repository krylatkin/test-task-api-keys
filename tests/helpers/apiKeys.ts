...
export function getApiKeyItem(page: Page, name: string, projectName: string): Locator {
  const nameMatcher = new RegExp(`^${escapeForRegex(name)}$`)

  if (projectName === 'mobile') {
    return page
      .locator('.card')
      .filter({ has: page.locator('.card__title-name', { hasText: nameMatcher }) })
      .first()
  }
...
export async function openActionsMenu(page: Page, name: string, projectName: string) {
